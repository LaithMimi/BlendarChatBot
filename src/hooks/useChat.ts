...
export const useChat = () => {
  ...
  const addWelcomeMessage = (preferences: UserPreferences) => {
    ...
    const existingSessionIndex = localChats.findIndex(
  ...
  const addLimitReachedMessage = () => {
    const limitMessage = {
      ...
      content:
        "הגעת למגבלת ההודעות החינמיות להיום. שדרג לחשבון פרימיום להודעות ללא הגבלה ותכונות נוספות. לחץ על הכפתור למטה כדי לשדרג ולהמשיך את מסע הלמידה שלך בערבית ללא הפרעות.",
      isUser: false,
      timestamp: new Date(),
    };
    ...
    // Check if we already have a chat for this user
    const existingSessionIndex = localChats.findIndex(
      (chat: any) => chat.userId === userIdEncoded
    );
  ...
  const handleSendMessage = async (content: string) => {
    ...
    if (limitReached) {
      ...
      toast({
        ...
        description:
          "You've reached the maximum messages for the free plan. Please upgrade to premium for unlimited messages.",
      });
    ...
    try {
      ...
      const response = await askQuestion(
        content,
        storedPrefs.week || userPreferences.week,
        storedPrefs.level || userPreferences.level,
        storedPrefs.gender || userPreferences.gender,
        storedPrefs.language || userPreferences.language
      );
      ...
      if (response.maxLimitReached) {
        ...
        toast({
          title: "Message limit reached",
          description:
            "You've reached the maximum messages for the free plan. Upgrade to Premium for unlimited messages.",
      ...
      if (response.limitWarning) {
        ...
        toast({
          ...
          description: `You have ${response.remainingMessages} message${
            response.remainingMessages === 1 ? "" : "s"
          } left. Consider upgrading to premium for unlimited messages.`,
        });
      ...
      if (
        ...
        const authErrorMessage: Message = {
          id: Date.now().toString(),
          content:
            "I'm having trouble authenticating your session. Please try logging in again or refreshing the page. If this error persists, check that your session hasn't expired.",
          isUser: false,
          timestamp: new Date(),
        };
        ...
        toast({
          title: "API Error",
          description:
            "Could not get a response. Please check the console for details.",
        });
      }
    } finally {
      setIsTyping(false);
